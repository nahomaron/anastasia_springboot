...
public class TenantPlanBillingCatalog {
    ...
    public PlanPrice resolve(SubscriptionPlan plan) {
        ...
        if (definition == null) {
            throw new IllegalStateException("No billing price configured for plan " + plan);
        }
        if (definition.getPriceId() == null || definition.getPriceId().isBlank()) {
            throw new IllegalStateException("Stripe priceId is missing for plan " + plan);
        }
        if (definition.getAmountMinor() == null || definition.getAmountMinor() < 0) {
            throw new IllegalStateException("amountMinor is invalid for plan " + plan);
        }
        return definition;
    }
